...
import {
  ...
  CreditCard, Activity, TrendingUp, AlertTriangle, CheckCircle2,
  Clock, Filter
} from 'lucide-react';
...
function getHealthScore(bills: number, products: number, users: number, daysSinceJoin: number) {
  if (daysSinceJoin < 3) return { score: 'new', label: 'New', color: 'bg-blue-100 text-blue-700' };
  const activityScore = Math.min(bills * 3 + products * 2 + users, 100);
  if (activityScore >= 30) return { score: 'healthy', label: 'Healthy', color: 'bg-emerald-100 text-emerald-700' };
  if (activityScore >= 10) return { score: 'moderate', label: 'Moderate', color: 'bg-amber-100 text-amber-700' };
  return { score: 'inactive', label: 'At Risk', color: 'bg-red-100 text-red-700' };
}
...
export default function TenantsTab() {
  ...
  const { data: businesses = [], isLoading: loadingBiz } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('businesses')
        .select('id, business_name, owner_id, mobile_number, created_at')
  ...
  const { data: profiles = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('profiles').select('business_id');
      return data || [];
    },
  });
  ...
  const { data: billCounts = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('bills').select('business_id');
      return data || [];
    },
  });
  ...
  const { data: productCounts = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('products').select('business_id');
      return data || [];
    },
  });
  ...
  const tenants = useMemo(() => {
    ...
    profiles.forEach((p: any) => {
      if (p.business_id) userCountMap.set(p.business_id, (userCountMap.get(p.business_id) || 0) + 1);
    });

    const billCountMap = new Map<string, number>();
    billCounts.forEach((b: any) => {
      if (b.business_id) billCountMap.set(b.business_id, (billCountMap.get(b.business_id) || 0) + 1);
    });

    const prodCountMap = new Map<string, number>();
    productCounts.forEach((p: any) => {
      if (p.business_id) prodCountMap.set(p.business_id, (prodCountMap.get(p.business_id) || 0) + 1);
    });
    ...
    return businesses.map(b => {
      ...
      const health = getHealthScore(bills, products, users, daysSinceJoin);

      return {
        ...b,
        sub,
        planName: sub?.plan_name || 'None',
  ...
  const stats = useMemo(() => {
    ...
    const noSub = tenants.filter(t => t.subStatus === 'none').length;
    ...
    return { total, active, trial, expired, noSub, avgBills, avgProducts };
  }, [tenants]);
  ...
  const handleExport = () => {
    ...
    const rows = filtered.map(t => ({
      ...
      'Products': t.products,
      'Team': t.users,
      'Joined': format(new Date(t.created_at), 'yyyy-MM-dd'),
      'Days Active': t.daysSinceJoin,
    }));
    const ws = XLSX.utils.json_to_sheet(rows);
    ws['!cols'] = [{ wch: 25 }, { wch: 14 }, { wch: 14 }, { wch: 10 }, { wch: 10 }, { wch: 8 }, { wch: 10 }, { wch: 6 }, { wch: 12 }, { wch: 10 }];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Shop Tenants</h2>
          <p className="text-sm text-muted-foreground mt-1">Monitor tenant health, engagement, and subscription status.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Tenants', value: stats.total, icon: Building2, color: 'text-blue-600 bg-blue-500/10' },
          { label: 'Active', value: `${stats.active} (${stats.total > 0 ? ((stats.active / stats.total) * 100).toFixed(0) : 0}%)`, icon: CheckCircle2, color: 'text-emerald-600 bg-emerald-500/10' },
          { label: 'Avg Bills/Shop', value: stats.avgBills, icon: FileText, color: 'text-violet-600 bg-violet-500/10' },
          { label: 'Avg Products', value: stats.avgProducts, icon: Package, color: 'text-amber-600 bg-amber-500/10' },
        ].map(item => (
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search by business name or phone..." className="pl-9" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
                    ...
                    <TableRow key={t.id} className="hover:bg-muted/30">
                      <TableCell>
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-sm">{t.business_name}</p>
                            <p className="text-[10px] text-muted-foreground">{t.mobile_number || 'No phone'}</p>
                          </div>
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <Badge className={cn('text-[10px]', t.health.color)}>{t.health.label}</Badge>
                      </TableCell>
