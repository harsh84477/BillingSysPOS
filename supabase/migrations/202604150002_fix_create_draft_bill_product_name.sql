-- Fix: Ensure create_draft_bill properly handles product_name from JSON items
-- This replaces the function with the latest version that includes:
-- 1. created_by = auth.uid()
-- 2. mrp_price + items_per_case support
-- 3. Proper jsonb_to_recordset with all fields
...
-- Also fix update_draft_bill with same COALESCE safety
